<?php
	...
	if(PHP_SAPI == "cli"){
		...
		*************************/
		$Config->setDB("alebal");					    // database
		$Config->setHOST("localhost");					    // database host
		$Config->setUSER("root");					        // user name
		$Config->setPWD("");						        // password
		$Config->setPATH("C:/Users/t.dulon/Documents/PERSO/alebal/Data/");			// The path where entities and repositories will be created
		$Config->initialize();
		...
		$Config->setLink(array(
			...
			'competitor' => array('event' => 'OneToMany'),
			'event' => array('competitor' => 'OneToOne')
		));
		/*******
		* END *
		*******/
